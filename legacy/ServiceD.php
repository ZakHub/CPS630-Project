...
<head>
  ...
  <title>GQZ TRAVELS - Luxury Travel Service</title>
<?php if (isset($_SESSION['id'])): ?>
  ...
  <script>
    ...
    function populateStores() {
      ...
      xhttp.open('GET', 'api/retrieveRacer.php', true);
      xhttp.onreadystatechange = function () {
        ...
        for (var racer of response) {
          target.innerHTML +=
            '<div id="store-'+racer.id+'" class="floating store">'+
              '<h3>'+racer.racerName+'</h3>'+
            '</div>'+
            '<br />';
          populateProducts(racer, 'store-'+racer.id);
        }
      };
      xhttp.timeout = 2000;
    ...
    function populateProducts(racer, target_id) {
      var xhttp = new XMLHttpRequest();
      xhttp.open('GET', 'api/retrieveLuxuryCar.php?id=' + racer.id, true);
      xhttp.onreadystatechange = function () {
        if (this.readyState !== 4 || this.status !== 200) {
          return;
        }
        ...
        var productTableHTML =
          '<table class="centered" style="width: 100%;">'+
            '<tr>'+
              ...
              '<th>Drag to cart</th>'+
            '</tr>';
        for (var luxury of response) {
          //console.log(target);
          productTableHTML +=
            //'<p>' + product.description + '</p>';
            '<tr>'+
              '<td>'+luxury.model+'</td>'+
              '<td>$'+luxury.rate.toFixed(2)+'</td>'+
              '<td class="draggable noselect" draggable="true" '+
                //'ondragstart="drag(event);" data-id="'+product.id+'">'+
                'ondragstart="drag(event);" data-json=\''+JSON.stringify(luxury)+'\'>'+
                'Drag Me'+
              '</td>'+
            '</tr>';
        }
      ...
      xhttp.timeout = 2000;
      xhttp.ontimeout = function (e) {
        alert('Failed to retrieve product list for ' + racer.racerName +
    ...
    function addToCart(productJSON) {
      ...
      const request = {
        type: 'LuxuryCar',
        content: JSON.parse(luxurycarJSON)
      };
      var xhttp = new XMLHttpRequest();
